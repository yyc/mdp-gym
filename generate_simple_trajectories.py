...
def compute_trajectory():
    ...
    def move_to_goal(pos, goal):
        while pos != goal:
            ...
            pos = observation['observation'][0]
    move_to_goal(observation['observation'][0], observation['achieved_goal'])
    record(traj, observation, 4)  # Grab
    observation, reward, done, info = env.step(4)
    move_to_goal(observation['observation'][0], observation['desired_goal'])
